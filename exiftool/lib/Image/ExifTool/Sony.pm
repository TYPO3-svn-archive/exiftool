...
%Image::ExifTool::Sony::Main = (
    ...
    0x2010 => {
        Name => 'Sony_0x2010',
        Unknown => 1, Hidden => 1,
        RawConv => sub { Cipher(\$_[0], 1); return $_[0] },
        ValueConv => 'PrintHex($val)',
        # notes for data in this block:
        # 0x00 - int32u: SequenceShutterCount
...
#
    ...
    0x9401 => {
        ...
        ValueConv => 'PrintHex($val)',
        # notes for data in this block:
        # 0x17 - 0=?, 1=normal shots,2=special effects,5=face detection
    },
    0x9402 => [{
        Name => 'Tag9402',
    ...
    0xb043 => { #9
        Name => 'AFMode',
        Writable => 'int16u',
        Condition => '$$self{Model} !~ /^DSC-RX100$/', # <-- yes, but which other models too???
        RawConv => '$val == 65535 ? undef : $val',
    ...
    # 0xb050 - int16u: 65535 (2 for HX9V, ref 12)
    # 0xb051 - int16u: 0
    0xb052 => { #PH (TX1)
...
%Image::ExifTool::Sony::Tag9400 = (
    ...
    0x0008 => { #PH
        Name => 'SequenceShutterCount',
        Notes => 'number of shutter releases in burst sequence',
        Format => 'int32u',
        ValueConv => '$val + 1',
        ValueConvInv => '$val - 1',
    },
    ...
    0x0010 => {
        ...
        PrintConv => {
            ...
            # 13 - ReleaseMode3 is 'Continuous - iSweep Panorama' (WX9 MPO image - PH)
...
#------------------------------------------------------------------------------
# Encipher/decipher Sony tag 0x94xx data
# Inputs: 0) data reference, 1) true to decipher the data
sub Cipher($;$)
{
    my ($dataPt, $decipher) = @_;
    # do the translation:
    # $o = ($u*$u*$u) % 249, where $o is the enciphered byte
    # (bytes with values 249-255 are not translated, and 0-1, 82-84, 165-167 and 248
    #  have the same enciphered value)
    # - use a hardcoded translation table for speed
    if ($decipher) {
